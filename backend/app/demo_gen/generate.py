...
DOCUMENTS = (
    DocSpec("initial", "initial", "01_Patient_ID.png", "Patient ID (insurer health e-card)", "patient_id",
            lambda: card_png(templates.patient_id_card())),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        print(f"{'FAILED' if problems else 'OK'}: {len(data.files)} documents checked in {args.out}")
    ...
    print(f"Wrote {len(data.files)} documents to {args.out}")
    return 0
